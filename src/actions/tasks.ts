 ...
 */
export async function getRecommendations(lat: number, lng: number): Promise<Recommendation[]> {
  ...
  if (error || !data) {
    console.error('[getRecommendations] Supabase query failed:', error?.message);
    return [];
  }

  const withinRadius = data
    .filter(hasValidCoordinates)
    .map((task) => ({ task, distanceKm: haversineKm(lat, lng, task.lat, task.lng) }))
    .filter((entry) => entry.distanceKm <= RECOMMENDATION_RADIUS_KM);

  // Same van, same date, both within radius: only the closer stop is a useful slot.
  const closestPerVehicleDate = new Map<string, Recommendation>();
  for (const entry of withinRadius) {
    const key = `${entry.task.vehicle_id}|${entry.task.scheduled_date}`;
    const existing = closestPerVehicleDate.get(key);
    if (!existing || entry.distanceKm < existing.distanceKm) {
      closestPerVehicleDate.set(key, entry);
    }
  }

  return [...closestPerVehicleDate.values()].sort((a, b) => a.distanceKm - b.distanceKm);
}
